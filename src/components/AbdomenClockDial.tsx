...
export function AbdomenClockDial({ selected, onSelect }: { selected?: string; onSelect: (label: string) => void }) {
  return (
    <svg viewBox="0 0 220 220" className="w-full max-w-[300px] mx-auto" role="group" aria-label="Abdomen clock positions">
      ...
      {ABDOMEN_CLOCK.map(({ label, hour }) => {
        ...
        return (
          <g key={label} className="cursor-pointer" onClick={() => onSelect(label)}>
            <circle cx={x} cy={y} r="20" fill={isSel ? '#22c55e' : '#334155'} opacity={isSel ? 0.5 : 0.25} />
